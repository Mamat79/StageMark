...
**Paquet Windows remplacé le 7 septembre 2026, même version 2027.0.2.**
Révision produit : `731d394`. Les anciens tags sont conservés ; le nouveau build
...
- En-tête SiLeMIO compact : slogan en infobulle, davantage d'espace de dessin ; logo, menus et commandes AFFICHER/BLACKOUT conservés. Repli adapté aux fenêtres étroites.
- Les noms de sources LIVE sont actualisés indépendamment des alertes et de leurs acquittements locaux. Les noms personnalisés restent préservés ; l'adoption du patch/plan CAD complet reste manuelle.
- Livraison des acquittements de commandes locales Windows fiabilisée ; aucun changement de droits ou de priorité du blackout.
...
- Notices StageMark FR/EN actualisées (14 pages chacune), Guide = notice produit et Aide > Guide de la suite = guide commun SiLeMIO 2027.3, embarqué à l'identique avec son manifeste d'empreintes.
...
- `StageMark-2027.0.2-Setup.exe` — **118 746 015 octets**.
...
Les vérifications logicielles incluent 882 tests Vitest et 15 tests Node,
trois parcours mobiles isolés dont deux handoffs avec le vrai producteur
StageFlow, et la comparaison de 72 fichiers runtime et 117 fichiers du paquet
Windows. Les notices FR/EN (14 pages chacune) sont rendues et relues.
L'en-tête passe 24 dispositions isolées FR/EN clair/sombre ; sa présentation
installée a été vérifiée sur un projet vide, sans activer la projection.
...
**Windows files replaced on 7 September 2026, retaining version 2027.0.2.**
Product revision: `731d394`. Existing tags remain intact; the new build is
...
- Compact SiLeMIO header: tagline moved to a tooltip, more drawing space, existing logo, menus and SHOW/BLACKOUT controls preserved. Controls wrap on narrow windows.
- LIVE source labels update independently of notifications and local acknowledgement. Custom names are preserved; adopting the complete Patch/CAD revision remains manual.
- Reliable Windows local-command acknowledgement delivery, without changing permissions or blackout priority.
...
- Updated French/English StageMark manuals (14 pages each): Guide opens the product manual; Help > Suite guide opens the canonical SiLeMIO 2027.3 guide, included unchanged with its checksum manifest.
...
This release contains the Windows x64 installer (**118,746,015 bytes**) and its
...
Windows-only. Checks include 882 Vitest and 15 Node tests, three mobile paths
(both handoffs use the real StageFlow producer), exact runtime/installer contents
and both 14-page manuals. The header passes 24 isolated layouts and a native
installed empty-project check without activating output. Native snapshot restoration is tested, not the installed
